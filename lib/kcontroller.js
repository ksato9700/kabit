(function() {
  ...
  KController = (function() {
    ...
    KController.prototype.send_touch = function(type, event) {
      var data, touch;
      touch = event.touches[0];
      if (touch) {
        data = [touch.screenX / this.width, touch.screenY / this.height];
      } else {
        data = null;
      }
    ...
    KController.prototype.sense_touch = function() {
      var _this = this;
      addEventListener('touchstart', function(event) {
        return _this.send_touch('touchstart', event);
      });
      addEventListener('touchmove', function(event) {
        return _this.send_touch('touchmove', event);
      });
      return addEventListener('touchend', function(event) {
        return _this.send_touch('touchend', event);
      });
    };
